                ...
                <div v-for="n in existingRoleCounter" class="row g-3 py-3 align-items-center">
                    ...
                    <div class="col-auto">
                        <select @change="appendCourseID" multiple size="10" v-model="selectedCourses" class="form-select select-skill" style="background-color: #d8648b; color:white;" >
                            <option v-for="course in coursesList" :value="course.Course_ID">{{course.Course_Name}}</option>
                        </select>
...
<script>
    export default{
        ...
        assignCourse: async function() {
            ...
            var assignmentList = []
            await axios.get('http://localhost:5000/skill_to_course/' + this.selectedSkills + '/')
            .then(response => {
            ...
            for (let i = 0; i < this.selectedCourses.length; i++) {
                for (let j = 0; j < assignmentList.length; j++){
                    ...
                    console.log(assignmentList[j].Course_ID)
                    if (assignmentList[j].Course_ID == this.selectedCourses[i]) {
                        exist = true;
                        }
                }
                }
            ...
            if (exist == false) {
            var skillID = this.selectedSkills
            for (let j = 0; j < this.selectedCourses.length; j++) {
                let courseID = this.selectedCourses[j]
